...
def get_wind_data_hourly(days=8):
    ...
    for attempt in range(3):
        try:
            ...
            r.raise_for_status() # エラーなら例外を発生させる
...
def get_real_tide_data(target_date):
    ...
    try:
        r = requests.get(url, headers=headers, timeout=5) # こちらもタイムアウト延長
...
with st.spinner("データを解析しています..."):
    ...
    if not wind_data:
        st.error("⚠️ 気象データの取得に失敗しました。しばらく待ってからブラウザを更新（リロード）してみてください。")
        st.stop()
