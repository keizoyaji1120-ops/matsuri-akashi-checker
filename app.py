...
@st.cache_data(ttl=3600) # キャッシュを1時間に設定（API制限対策）
def get_wind_data_hourly(days=8):
    """Open-Meteoから週間風予報を取得 (429エラー強力対策版)"""
    ...
    for attempt in range(5):
        try:
            ...
            # 429 (Too Many Requests) の場合は長く待つ
            if e.response.status_code == 429:
                last_error = f"アクセス集中 (429): {e}"
                wait_time = 5 + (attempt * 3) # 5秒, 8秒, 11秒...と待つ時間を増やす
...
with st.spinner("気象データを解析しています..."):
    ...
    if wind_data is None or len(wind_data) == 0:
        ...
        if error_msg:
            st.warning("現在アクセスが集中しています。")
            with st.expander("詳細エラーログ"):
                st.code(str(error_msg))
            st.info("💡 1分ほど時間を空けてから、再度リロード（更新）してみてください。")
        st.stop()
...
"""
    ...
    st.warning(f"⚠️ {selected_date.strftime('%m/%d')} の予報データが見つかりませんでした。")
...
with st.expander("⚠️ 免責事項 (必ずお読みください)"):
    ...
    <p><strong>5. 営利利用の禁止</strong><br>
    本アプリを営利目的で使用すること、および販売・有償提供することを固く禁止します。</p>
    </div>
...
<div style="text-align: center; font-size: 0.8em; color: #888;">
    <p>© 2026 魔釣 - Matsuri Fishing Forecast</p>
